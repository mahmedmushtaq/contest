...
public class CheckWhoSawReq {
    ...
    public void whoSawData(final Context context, String postId, String lastId){
        OkHttpClient client = new OkHttpClient();
        RequestBody body = new FormBody.Builder().add("post_id", postId).add("last_id",lastId).build();
        Request request = new Request.Builder().post(body).url(Addresses.getWebAddress()+ VoteFiles.whoSawPost_file).build();
                        ...
                        try {
                            ...
                            if(Validator.validateWebResult(result)){
                                ...
                                for(int i = 0; i < namesArray.length(); i++){
                                    ...
                                    mArrayHolder.setCheckWhoSawData(data);
...
//                                rv.notifyDataSetChanged();
                                ...
                                if(mOnLoadingCheckSawAndVoteData != null) {
                                    ((Activity)context).runOnUiThread(new Runnable() {
                                        @Override
                                        public void run() {
                                            mOnLoadingCheckSawAndVoteData.load(mArrayHolder.getCheckWhoSawData());
                                        }
                                    });
