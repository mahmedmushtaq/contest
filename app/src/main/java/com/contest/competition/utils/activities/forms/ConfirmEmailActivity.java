...
public class ConfirmEmailActivity extends AppCompatActivity {
    ...
    private void againSendConfirmEmail(){
        Dialoger.setDialog(ConfirmEmailActivity.this,"Confirm email","Please wait.Email is sending again");
        OkHttpClient client = new OkHttpClient();
        final RequestBody body = new FormBody.Builder().add("email",email).build();
        Request request = new Request.Builder().url(Addresses.getWebAddress()+ FormsFiles.againSendConfirmEmail).post(body).build();
    ...
    private void cnfrmEmail() {
        Dialoger.setDialog(ConfirmEmailActivity.this,"Confirm email","Please wait process is doing");
        OkHttpClient client = new OkHttpClient();
//        check = update mean update users that email is check
        ...
        Request request = new Request.Builder().url(Addresses.getWebAddress()+ FormsFiles.confirmCreateNewUsers).post(body).build();
        Call call = client.newCall(request);
        call.enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                final String body = response.body().string();
                Log.e("cnfirm email", "onResponse: "+body );
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        try {
    ...
    public void onBackPressed() {
        super.onBackPressed();
        Animatoo.animateSlideRight(ConfirmEmailActivity.this);
    }
}
